...
#if !defined(KRATOS_LAYER_APP_NONUNIFORM_STRUCTURED_GRID_ELEMENTAL_INDEXING_H_INCLUDED )
#define KRATOS_LAYER_APP_NONUNIFORM_STRUCTURED_GRID_ELEMENTAL_INDEXING_H_INCLUDED
 ...
 */
template<int TDim>
class NonuniformStructuredGridElementalIndexing : public MeshQueryTool<Element>
{
public:
    ///@name Type Definitions
    ///@{
    ...
    /// Pointer definition
    KRATOS_CLASS_POINTER_DEFINITION(NonuniformStructuredGridElementalIndexing);
    ...
    /// Default constructor.
    NonuniformStructuredGridElementalIndexing(const double& Tolerance)
    : BaseType()
    {
        ...
        Element::Pointer& pMatchedMaster, CoordinatesArrayType& local_coords) const final
    {
        for (unsigned int i = 0; i < master_elements.size(); ++i)
        {
            ...
            if (it == rAllElements.end())
                KRATOS_ERROR << "The master element Id is not in the list of element";
...
private:
    ...
    /// Assignment operator.
    NonuniformStructuredGridElementalIndexing& operator=(NonuniformStructuredGridElementalIndexing const& rOther)
    {
        return *this;
    }

    /// Copy constructor.
    NonuniformStructuredGridElementalIndexing(NonuniformStructuredGridElementalIndexing const& rOther)
    {
    }

    ///@}

}; // Class NonuniformStructuredGridElementalIndexing
...
//inline std::istream& operator >>(std::istream& rIStream, NonuniformStructuredGridElementalIndexing& rThis)
//{
//    return rIStream;
//}
...
//        const NonuniformStructuredGridElementalIndexing& rThis)
//{
...
#endif // KRATOS_LAYER_APP_NONUNIFORM_STRUCTURED_GRID_ELEMENTAL_INDEXING_H_INCLUDED defined